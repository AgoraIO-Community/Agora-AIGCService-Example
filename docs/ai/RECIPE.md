# AIGC service example with RTC

## Overview

Connect an AIGC service workflow to an Agora RTC media session for interactive AI-assisted experiences.

Use this recipe when you want a working reference for service integration, RTC media, AIGC pipeline, backend setup.

Repository: [AgoraIO-Community/Agora-AIGCService-Example](https://github.com/AgoraIO-Community/Agora-AIGCService-Example)
...
For service examples, use server-generated tokens and keep App Certificate values only on the backend.

## Run The Sample

Install the service dependencies, configure Agora credentials, then start the sample service. Connect it to a test RTC channel and verify the service receives or publishes the expected media workflow.
...
```text
You are working in `AgoraIO-Community/Agora-AIGCService-Example`. Add or adapt this sample so a developer can run the aigc service example with rtc end to end with Agora RTC.
